...
/// Type registers to deserialize `StatesCurrentFile` and `StatesDesiredFile`.
#[derive(Debug, Default)]
pub struct StatesTypeRegs {
    /// Type register for each item spec's `State<StateLogical, StatePhysical>`.
    states_current_type_reg: TypeReg<ItemSpecId>,
    /// Type register for each item spec's `StateLogical`.
...
impl StatesTypeRegs {
    ...
    /// Returns a reference to the current states type register.
    ///
    ...
    /// Returns a mutable reference to the current states type register.
    ///
    /// This maps from each item spec's ID to `State<StateLogical,
    /// StatePhysical>`.
    ...
    /// Returns a reference to the desired states type register.
    ///
    ...
    /// Returns a mutable reference to the desired states type register.
    ///
    /// This maps from each item spec's ID to `StateLogical`.
