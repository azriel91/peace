            ...
            fields_named
                ...
                .fold(
                    ...
                    |mut tokens, (field, field_name)| {
                        ...
                        let resolve_method = resolve_mode.resolve_method(field_name);
            ...
            fields_unnamed
                ...
                .fold(
                    ...
                    |mut tokens, (field_index, field)| {
                        ...
                        let field_ty = &field.ty;
                        let resolve_method = resolve_mode.resolve_method(&field_ident);
...
impl<'name> ResolveMode<'name> {
    ...
    // resolution.
    fn resolve_method(self, field_name: &Ident) -> proc_macro2::TokenStream {
        match self {
            Self::Full { .. } => quote! {
                let #field_name = #field_name.resolve(resources, value_resolution_ctx)?;
            },
            Self::Partial { .. } => quote! {
                let #field_name = #field_name.resolve_partial(resources, value_resolution_ctx)?
                    .try_into()
                    .ok();
            },
        }
