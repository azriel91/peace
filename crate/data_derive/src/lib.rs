...
fn impl_data_access(ast: &DeriveInput) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        impl #impl_generics #peace_data_path::Data< #impl_borrow_lt >
            ...
            #where_clause
        {
            fn borrow(step_id: & #impl_borrow_lt #peace_cfg_path::StepId, resources: & #impl_borrow_lt #peace_data_path::Resources) -> Self {
                #borrow_return
            }
        }
    }
    ...
    let (field_names_tokens, borrow_return) = match data_type {
        DataType::Struct => {
            ...
            let borrow_return = quote! {
                #name {
                    #( #field_names_tokens: Data::borrow(step_id, resources) ),*
        ...
        DataType::Tuple => {
            ...
            let borrow = vec![quote! { Data::borrow(step_id, resources) }; count];
            let borrow_return = quote! {
                #name ( #( #borrow ),* )
            };
