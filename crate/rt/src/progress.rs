...
impl Progress {
    ...
    pub async fn progress_render<E, O>(
        ...
        mut progress_close_rx: oneshot::Receiver<()>,
    ) where
        O: OutputWrite<E>,
    {
        loop {
            tokio::select! {
                ...
                oneshot = &mut progress_close_rx => {
                    // Whether we're told to end, or the sender drops, we will end.
                    let (Ok(()) | Err(_)) = oneshot;
                    break;
                }
                progress_update_and_id = progress_rx.recv() => {
                    match progress_update_and_id {
                        Some(progress_update_and_id) => {
