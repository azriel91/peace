...
async fn reads_states_desired_from_disk_when_present() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cmd_ctx = CmdCtx::builder_single_profile_single_flow::<PeaceTestError>(
        &mut fn_tracker_output,
        &workspace,
    )
    .with_profile(profile!("test_profile"))
    .with_flow(Flow::new(FlowId::new(crate::fn_name_short!())?, graph))
    .await?;
    let cmd_ctx = StatesDesiredDisplayCmd::exec_v2(cmd_ctx).await?;
...
async fn returns_error_when_states_not_on_disk() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .with_flow(Flow::new(FlowId::new(crate::fn_name_short!())?, graph))
    .await?;
    let exec_result = StatesDesiredDisplayCmd::exec_v2(cmd_ctx).await;
