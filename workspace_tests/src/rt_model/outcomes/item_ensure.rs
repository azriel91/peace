...
-> Result<(), Box<dyn std::error::Error>> {
    let item_ensure_partial = ItemEnsurePartial {
        ...
        state_desired: Some(State::new(Nothing, External::tbd())),
        state_diff: Some(8u8),
    ...
    assert_eq!(
        ItemEnsure {
            ...
            state_desired: State::new(Nothing, External::tbd()),
            state_diff: 8u8,
...
fn item_ensure_rt_state_desired_returns_state_desired() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        State::new(Nothing, External::tbd()),
        *state_desired
            .downcast::<State<Nothing, External>>()
            .unwrap()
    );
    Ok(())
}
